...
namespace SampleEntityFramework {
    internal class Program {
        ...
        static void DisplayAllBooks3() {
            using (var db = new BooksDbContext()) {
                var book = db.Books.Where(a=> a.Title.Length==db.Books.Max(b=>b.Title.Length));
        ...
        private static void Exercise1_4() {
            using (var db = new BooksDbContext()) {
                var groups = db.Books.OrderBy(a=>a.PublishedYear).Include(nameof(Author)).Take(3);
                foreach (var group in groups) {
                    Console.WriteLine($"{group.PublishedYear}:{ group.Title} {group.Author.Name}");
                }
            }
        }
        ...
        private static void Exercise1_5() {
            using (var db = new BooksDbContext()) {
                var birthday = db.Authors.OrderByDescending(a=>a.Birthday).ToList();
